CREATE OR REPLACE FUNCTION public.deck_analytics(p_deck_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  SELECT count(*) INTO total_learning
    FROM cards
   WHERE deck_id = p_deck_id
     AND card_state = 'learning';

  SELECT count(*) INTO completed_step1
    FROM cards
   WHERE deck_id = p_deck_id
     AND card_state = 'learning'
     AND learning_step >= 1;

  SELECT count(*) INTO completed_step2
    FROM cards
   WHERE deck_id = p_deck_id
     AND card_state = 'learning'
     AND learning_step >= 2;
  ...
  IF avg_lapse IS NULL THEN avg_lapse := 0; END IF;
  ...
  SELECT avg(ease_factor)::numeric INTO avg_ease
    FROM cards
   WHERE deck_id = p_deck_id;
  IF avg_ease IS NULL THEN avg_ease := 2.5; END IF;
  suggested_ease_adj := round((2.5 - avg_ease) * 0.1, 2);
  IF avg_ease < 2.2 THEN suggested_steps := ARRAY[1,10,30];
  ELSE suggested_steps := ARRAY[1,10]; END IF;
...
GRANT EXECUTE ON FUNCTION public.deck_analytics(uuid) TO authenticated; 