DROP FUNCTION IF EXISTS public.deck_learning_insights(uuid);
CREATE FUNCTION public.deck_learning_insights(p_deck_id uuid)
RETURNS jsonb
LANGUAGE sql
SECURITY DEFINER
AS $$
  WITH hourly AS (
    ...
    WHERE fatigue_score IS NOT NULL AND fatigue_score < 70
  ), overall AS (
